...
describe('PATCH /api/groups/:groupId/events/:eventId/logistics/:itemId', () => {
  ...
  it('includes item_date in the update payload sent to the service', async () => {
    ...
    expect(logisticsService.updateLogisticsItem).toHaveBeenCalledWith(
      'event-1', 'group-1', 'item-1', 'creator-1', { item_date: '2026-09-23' }
    );
  });
  ...
  it('returns 400 when item_date is not a string or null', async () => {
    ...
    expect(res.status).toBe(400);
    expect(logisticsService.updateLogisticsItem).not.toHaveBeenCalled();
  });
});
